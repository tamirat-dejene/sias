...
import { students, enrollments, courses, grades, instructors, dacShares, users, auditLogs } from "@/db/schema";
...
async function getCurrentUser() {
    ...
    return session?.user as typeof users.$inferSelect | undefined;
}
...
export async function getUsersForSharing() {
     ...
     return await db.select({
         ...
         name: users.name,
         role: users.role
     }).from(users).where(ne(users.id, user.id));
}


export async function shareResource(resourceId: number, resourceType: string, targetUserId: string, permission: "read" | "write") {
    const user = await getCurrentUser();
...
export async function getAdminStats() {
    ...
    const [instructorCount] = await db.select({ count: instructors.id }).from(instructors);
